...
def show_select_view(excel_path_dir,generate_xml_dir,generate_csharp_dir,export_all):
    ...
    if export_all:
        for i in range(len(lst_file_name)):
            select_indexs.append(i + 1)
            print("append index", i)
    else:
        repeat = True
        while repeat:
            select_indexs.clear()
...
def csharp_parse_float_property(first_line,key):
    if first_line == "required" or first_line == "required_key":
        return "\t\t\t{0} = float.Parse(node.Attribute(\"{1}\"));".format(key, key)
    elif first_line == "repeated":
        temp_str = "\t\t\t{0} = new List<float>();\n".format(key)
        temp_str += "\t\t\tstring str_{0} = node.Attribute(\"{1}\");\n".format(key,key)
        temp_str += "\t\t\tif(!string.IsNullOrEmpty(str_{0}))\n".format(key)
        temp_str += "\t\t\t{\n";
        temp_str += "\t\t\t\tstring[] {0}Arr = str_{1}.Split(\',\');\n".format(key,key)
        ...
        temp_str += "\t\t\t\t\tfor (int i = 0; i < {0}Arr.Length; i++)\n".format(key)
        temp_str += "\t\t\t\t\t{\n"
        temp_str += "\t\t\t\t\t\t{0}.Add(float.Parse({1}Arr[i]));\n".format(key,key)
        temp_str += "\t\t\t\t\t}\n"
        temp_str += "\t\t\t\t}\n"
        temp_str += "\t\t\t}"
        return temp_str
...
def csharp_float_property(key,first_line):
    if first_line == "required":
        return "public float "+key+" { get; private set; }"
    elif first_line == "required_key":
        return "[ResCfgKey]\n\t\tpublic float "+key+" { get; private set; }"
    elif first_line == "repeated":
        return "public List<float> " + key + " { get; private set; }"
    raise Exception("first_line = {0}的字符串编写错误".format(first_line))
...
if __name__ == "__main__":
    if len(sys.argv) > 2:
        ...
        export_all = False
        if len(sys.argv) > 4:
            export_all  = bool(sys.argv[4])
        show_select_view(excel_dir,xml_dir,csharp_dir,export_all)
